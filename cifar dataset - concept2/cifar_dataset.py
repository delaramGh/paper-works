...
################################################################################
################################################################################
# csv_file = "concept2_CIFAR10-c.csv"
...
################################################################################
################################################################################
csv_file2 = "concept2_CIFAR10-c.csv"
df2 = pd.read_csv(csv_file2)

csv_file1 = "C:\\Users\\ASUS\\Desktop\\research\\mitacs project\\paper experiments\\cifar dataset\\cifar_dataset_modified.csv"
...
for i in range(len(df1)):
    for j in range(len(df2)):
        if df1["gen"][i] == df2["gen"][j]:
            # print(df1.iloc[i])
            new_df.append(df1.iloc[i])
            label.append(df2["label"][j])
            # input("wait          ")
            # cnt+=1
            # break
# print(cnt)
new_df = pd.DataFrame(new_df)
new_df["correct label"] = label

new_df.to_csv("concep2_cifar_all.csv")
